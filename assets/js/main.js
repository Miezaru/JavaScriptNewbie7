...
let replaceCSSComments = (str) =>
  (str = str.replace(/\/\*.*?\*\/\s{0,}/gi, ''));
...
let replaceHTMLComments = (str) =>
  (str = str.replace(/<!--.*?-->\s{0,}/gi, '').trim());
 ...
 */
let validateFileType = (image) => /\.(png|jpeg|jpg)$/gi.test(image);
 ...
 */
let checkYear = (year) => /^(19\d\d|20\d\d|2100)$/gi.test(year);
 ...
 */
let checkEmail = (mail) => /^[\w-._]{1,20}@\w{2,10}\.[\w.]{2,20}$/gi.test(mail);
...
let checkDomainUrl = (domain) =>
  /^(http|https):\/\/[\w-_]{0,50}\.[\w-_.]{0,50}$/gi.test(domain);
 ...
 */
let createLinksFromDomains = (domain) => {
  let regularUrl = new RegExp(
    '(http|https)://[\\w-_]{2,50}\\.[\\w-_.]{2,50}',
    'gi'
  );
  let regularName = new RegExp('[\\w-_]{2,50}\\.[\\w-_.]{2,50}', 'gi');

  let domainUrl = domain.match(regularUrl);
  let domainName = [];
  for (let i = 0; i < domainUrl.length; i++) {
    domainName[i] = domainUrl[i].replace(/(http|https):\/\//gi, '');
  }
  for (let i = 0; i < domainUrl.length; i++) {
    domain = domain.replace(
      domainUrl[i],
      `<a href="${domainUrl[i]}" target="_blank">${domainName[i]}</a>`
    );
  }
  return domain;
};
